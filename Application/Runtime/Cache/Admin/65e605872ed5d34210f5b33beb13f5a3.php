              ...
              <table class="table table-bordered table-hover singcms-table">
                ...
                <tbody>
                  <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><?php echo (str_ireplace($search,"<font color='#ff000'>$search</font>",$data["username"])); ?></td>
                      <td><?php echo (long2ip($data["lastloginip"])); ?></td>
                      <td><?php echo (date('Y-m-d H:i:s',$data["lastlogintime"])); ?></td>
                      <td><?php echo ($data["email"]); ?></td>
                      <td><?php echo ($data["realname"]); ?></td>
                      <!-- 状态<td><span attr-status="<?php if($data["status"] == 1): ?>1<?php else: ?>0<?php endif; ?>"  attr-id="<?php echo ($data["user_id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off" ><?php echo (status($new["status"])); ?></span></td> -->
                      <td><?php echo (getUserType($data["type"])); ?></td>
                     <td>
                      <!--  <span class="sing_cursor glyphicon glyphicon-edit" aria-hidden="true" id="singcms-edit" attr-id="<?php echo ($data["admin_id"]); ?>" ></span> -->
                         <span class="sing_cursor glyphicon glyphicon-remove-circle" aria-hidden="true" id="singcms-delete"  attr-id="<?php echo ($data["admin_id"]); ?>"  attr-message="要删除，如想恢复请联系管理员"></span>
                      </td>
